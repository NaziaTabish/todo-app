...
def test_sc_003():
    ...
    try:
        ...
        print("   [FAIL] Create operation failed")
        return False
    ...
    try:
        ...
        if len(tasks) > 0:
            operations_tested += 1
            print("   [PASS] Read operation: 1 interaction")
        else:
            print("   [FAIL] Read operation failed: no tasks returned")
            return False
    except:
    ...
    try:
        ...
        if updated_task.title == "Updated CRUD Test Task":
            operations_tested += 1
            print("   [PASS] Update operation: 1 interaction")
        else:
            print("   [FAIL] Update operation failed: title not updated")
            return False
    except:
    ...
    try:
        ...
        if new_count == original_count - 1:
            operations_tested += 1
            print("   [PASS] Delete operation: 1 interaction")
        else:
            print("   [FAIL] Delete operation failed: task not deleted")
            return False
    except:
    ...
    try:
        ...
        if toggled_task.completed:
            operations_tested += 1
            print("   [PASS] Toggle operation: 1 interaction")
        else:
            print("   [FAIL] Toggle operation failed: not completed")
            return False
    except:
        print("   [FAIL] Toggle operation failed")
        return False

    if operations_tested == 5:
        print(f"   [PASS] SC-003 PASSED: All {operations_tested}/5 operations work with <=3 interactions")
        return True
    else:
        print(f"   [FAIL] SC-003 FAILED: Only {operations_tested}/5 operations passed")
        return False
...
def test_sc_004():
    ...
    try:
        ...
        print("   [FAIL] Invalid title test failed: exception not raised")
        return False
    except InvalidTitleException:
        print("   [PASS] Invalid title handled with clear error message")
    except Exception as e:
        print(f"   [FAIL] Unexpected error for invalid title: {e}")
        return False

    # Test invalid task ID (non-existent)
    try:
        todo_manager.toggle_completion(999)  # Should raise TaskNotFoundException
        print("   [FAIL] Invalid ID test failed: exception not raised")
        return False
    except TaskNotFoundException:
        print("   [PASS] Invalid task ID handled with clear error message")
    except Exception as e:
        print(f"   [FAIL] Unexpected error for invalid ID: {e}")
        return False

    # Test update with invalid ID
    try:
        todo_manager.update_task(999, title="Invalid update")  # Should raise TaskNotFoundException
        print("   [FAIL] Invalid update test failed: exception not raised")
        return False
    except TaskNotFoundException:
        print("   [PASS] Invalid update ID handled with clear error message")
    except Exception as e:
        print(f"   [FAIL] Unexpected error for invalid update: {e}")
        return False

    # Test delete with invalid ID
    try:
        todo_manager.delete_task(999)  # Should raise TaskNotFoundException
        print("   [FAIL] Invalid delete test failed: exception not raised")
        return False
    except TaskNotFoundException:
        print("   [PASS] Invalid delete ID handled with clear error message")
    except Exception as e:
        print(f"   [FAIL] Unexpected error for invalid delete: {e}")
        return False

    print("   [PASS] SC-004 PASSED: All invalid inputs handled with clear error messages")
    return True
...
def test_sc_005():
    ...
    try:
        ...
        if not completed_task.completed:
            print("   [FAIL] Full cycle failed: task not marked complete")
            return False
        ...
        if new_count != original_count - 1:
            print("   [FAIL] Full cycle failed: task not deleted")
            return False

        elapsed_time = time.time() - start_time

        if elapsed_time <= 120:  # 2 minutes = 120 seconds
            print(f"   [PASS] SC-005 PASSED: Full cycle completed in {elapsed_time:.2f}s (< 120s)")
            return True
        else:
            print(f"   [FAIL] SC-005 FAILED: Cycle took {elapsed_time:.2f}s (> 120s)")
            return False

    except Exception as e:
        print(f"   [FAIL] Full cycle failed with error: {e}")
        return False
